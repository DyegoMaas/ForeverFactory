...
namespace FactoryNet.Tests
{
    public class ManyInstanceFactoryTests
    {
        ...
        [Fact]
        public void WithNext_applies_transformation_only_over_the_next_x_instances_produced()
        {
            var persons = _factory
                .Many(count: 7)
                .WithFirst(count: 2, x => x.Age = 19)
                .WithNext(count: 2, x => x.LastName = "Nobel")
                .WithNext(count: 2, x => x.FirstName = "Clarice")
                .Build()
                .ToList();
            ...
            var nextTwo = persons.Skip(2).Take(2);
            ...
            nextTwo = persons.Skip(4).Take(2);
            foreach (var person in nextTwo)
            {
                person.FirstName.Should().Be("Clarice");
                person.LastName.Should().Be("Einstein");
                person.Age.Should().Be(56);
            }
            
            var lastOne = persons.Last();
            lastOne.FirstName.Should().Be("Albert");
            lastOne.LastName.Should().Be("Einstein");
            lastOne.Age.Should().Be(56);
        }
        
        [Fact]
        public void It_should_apply_all_types_of_modifications()
        {
            var persons = _factory
                .Many(count: 7)
                .With(x => x.Age = 101)
